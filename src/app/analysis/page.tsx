...
function AnalysisHistoryContent() {
        ...
        async function fetchAnalyses() {
            try {
                ...
                if (result.success && result.data) {
                    const transformedData = result.data.map((c: Record<string, unknown>) => ({
                        id: c.id as string,
                        fileName: (c.fileName || c.file_name) as string,
                        filePath: (c.filePath || c.file_path) as string | undefined,
                        contractType: (c.contractType || c.contract_type || 'General') as string,
                        sector: (c.sector || 'legal') as string,
                        effectiveDate: (c.effectiveDate || c.effective_date) as string | undefined,
                        renewalDate: (c.renewalDate || c.renewal_date) as string | undefined,
                        riskScore: (c.riskScore || c.risk_score) as number | undefined,
                        createdAt: (c.createdAt || c.created_at) as string | undefined,
                        extractedData: (c.extractedData || c.extracted_data || {}) as Record<string, unknown>,
                        abusiveClauses: (c.abusiveClauses || c.abusive_clauses || []) as (string | { reference?: string; explanation?: string; severity?: string })[],
                        alerts: (c.alerts || []) as (string | { message: string; severity: string })[],
                        summary: c.summary as string | undefined,
                        noticePeriodDays: (c.noticePeriodDays ?? c.notice_period_days ?? 30) as number,
                        terminationClauseReference: (c.terminationClauseReference || c.termination_clause_reference || '') as string,
                        parties: (c.parties || []) as string[],
                        tags: (c.tags || []) as string[],
                        customQuery: c.customQuery as string | undefined,
                        customAnswer: c.customAnswer as string | undefined,
                        requestedDataPoints: Object.keys((c.extractedData || c.extracted_data || {}) as Record<string, unknown>),
                    }));
    ...
    // Handlers for AnalysisResult component interactions
    const handleRename = async (id: string, newName: string) => {
        try {
            const response = await fetch('/api/contracts/update', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id, fileName: newName })
            });

            if (response.ok) {
                setAnalyses(prev => prev.map(a => a.id === id ? { ...a, fileName: newName } : a));
            }
        } catch (error) {
                ...
                <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    {activeTab === 'analyses' && (
                        <>
                            ...
                            {/* Featured Expanded Analysis Section - PRO UI */}
                            <AnimatePresence>
                                {expandedId && (
                                    <motion.div
                                    ...
                                    >
                                        {(() => {
                                            ...
                                            // Cast to ContractAnalysis type expected by component
                                            // The data mapping above ensures we have the fields
                                            return (
                                                <AnalysisResult
                                                    analysis={expandedAnalysis as any}
                                                    onDismiss={() => setExpandedId(null)}
                                                    onRename={handleRename}
                                                    onUpdateTags={handleUpdateTags}
                                                    fileAvailable={false} // Disable re-analysis for historical items
                                                />
                                            );
                                        })()}
                                    </motion.div>
